...
window.openOrderEditModal = async function(id) { 
    ...
    inputs.forEach(i => { if(i.id !== 'btn-save-progress' && i.id !== 'btn-save-approve' && i.id !== 'oed-status') i.disabled = isLocked; });
...
window.executeApprovalTransaction = async function(formData) {
    ...
    try {
        ...
        link.download = `Cotizacion_Aprobada_${folioUnificado}.pdf`;
        link.click();
...
window.confirmAndGeneratePurchaseOrder = async function() {
    window.openConfirm("¿Generar Orden de Compra Oficial? Se guardará una copia exacta.", async () => {
        ...
        try {
            ...
            const link = document.createElement('a'); link.href = URL.createObjectURL(pdfBlob); link.download = `OC_${folioUnificado}.pdf`; link.click();
...
window.downloadPDFFromPreview = function() { 
    const element = document.getElementById('pdf-content'); 
    const folioUnificado = currentPreviewOrder.numero_orden || currentPreviewOrder.id.split('-')[0].toUpperCase();
    const opt = { margin: 0, filename: `Documento_${folioUnificado}.pdf`, image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2, useCORS: true }, jsPDF: { unit: 'in', format: 'letter' } }; 
...
window.getOrderHTML = async function(o, type) { 
    const isOrder = type === 'order'; 
    let logoData = null; try { logoData = await getBase64ImageFromUrl(COMPANY_LOGO_URL); } catch(e) { console.warn("No se pudo pre-cargar el logo en Base64 para el PDF."); }
    const logoImg = logoData ? `<img src="${logoData}" style="height: 64px; object-fit: contain;">` : `<h2 style="font-size:24px; font-weight:bold; color:#D32F2F;">ADMINISTRACIÓN</h2>`; 
    ...
    let rowsHtml = `<tr><td class="py-2 px-3 align-top"><p class="font-bold text-gray-800 text-xs">${o.espacio_nombre}</p>${descHTML}<span class="bg-gray-100 text-gray-500 px-1 py-0.5 rounded text-[10px] font-mono mt-0.5 inline-block">${o.espacio_clave || ''}</span></td><td class="py-2 px-3 align-top text-center text-gray-500 text-xs">${window.safeFormatDate(o.fecha_inicio)}<br>${window.safeFormatDate(o.fecha_fin)}</td><td class="py-2 px-3 align-top text-right font-bold text-gray-700 text-xs">${new Intl.NumberFormat('es-MX', {style:'currency',currency:'MXN'}).format(rentalTotal)}</td></tr>`; 
