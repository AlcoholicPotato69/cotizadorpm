...
function calculateDayByDayTotal(space, startStr, endStr, guests) {
    ...
    let rules = [];
    if (space.precios_por_dia) { if (Array.isArray(space.precios_por_dia)) rules = space.precios_por_dia; else rules = [{ min: 0, max: 999999, precios: space.precios_por_dia }]; } else { rules = [{ min: 0, max: 999999, precios: {lunes: space.precio_base||0, martes:space.precio_base||0, miercoles:space.precio_base||0, jueves:space.precio_base||0, viernes:space.precio_base||0, sabado:space.precio_base||0, domingo:space.precio_base||0} }]; }
    const guestCount = parseInt(guests) || 1; let activeRule = rules.find(r => guestCount >= r.min && guestCount <= r.max);
    if (!activeRule) { if (guestCount > 0) activeRule = rules.reduce((prev, current) => (prev.max > current.max) ? prev : current); else activeRule = rules[0]; }
    const prices = activeRule ? (activeRule.precios || {}) : {};
    let total = 0; let details = [];
    const start = new Date(startStr + 'T00:00:00'); const end = new Date(endStr + 'T00:00:00'); const keys = ['domingo', 'lunes', 'martes', 'miercoles', 'jueves', 'viernes', 'sabado']; const names = ['Domingo', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado'];
    for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) { const dayIdx = d.getDay(); const key = keys[dayIdx]; const price = parseFloat(prices[key] || 0); total += price; details.push({ date: d.toLocaleDateString('es-MX'), dayName: names[dayIdx], price: price, ruleMin: activeRule?.min, ruleMax: activeRule?.max }); }
...
window.openConfirm = function(msg, callback, isWarning = false) { const titleEl = document.getElementById('confirm-title'); titleEl.innerHTML = isWarning ? `<i class="fa-solid fa-triangle-exclamation text-red-600 mb-2 text-2xl block"></i> ${msg}` : msg; confirmCallback = callback; window.openModal('generic-confirm-modal'); };
window.askDeleteOrder = function(id, e) { if(e) e.stopPropagation(); window.openConfirm("¿Eliminar cotización y TODOS sus archivos? Esta acción es irreversible.", async () => { try { window.showToast("Eliminando archivos asociados...", "info"); const { data: files } = await window.globalSupabase.storage.from('documentos').list(`${id}`, { limit: 100 }); if (files && files.length > 0) { await window.globalSupabase.storage.from('documentos').remove(files.map(x => `${id}/${x.name}`)); } const { data: receiptFiles } = await window.globalSupabase.storage.from('documentos-cp').list(`${id}/recibos`); if (receiptFiles && receiptFiles.length > 0) { await window.globalSupabase.storage.from('documentos-cp').remove(receiptFiles.map(x => `${id}/recibos/${x.name}`)); } await window.finSupabase.from('cotizaciones').delete().eq('id', id); window.showToast("Cotización y archivos eliminados", "success"); window.loadOrders(); } catch (err) { console.error(err); window.showToast("Error al eliminar: " + err.message, "error"); } }, true); };
...
function renderOrdersTable(data) {
    ...
    data.forEach(o => {
        ...
        tr.innerHTML = `<td class="p-4">${o.numero_orden || '--'}<p class="text-[9px] text-gray-400">ID: ${o.id.slice(0,6)}</p></td><td class="p-4 font-bold text-xs text-gray-700">${o.cliente_nombre}</td><td class="p-4 text-xs"><span class="font-bold block">${o.espacio_nombre}</span><span class="text-gray-500 font-mono">${window.safeFormatDate(o.fecha_inicio)}</span></td><td class="p-4 text-right font-mono font-bold text-xs">${new Intl.NumberFormat('es-MX', {style:'currency',currency:'MXN'}).format(o.precio_final)}</td><td class="p-4 text-center"><span class="${sColor} px-2 py-1 rounded text-[9px] font-black uppercase tracking-wider">${sText}</span>${alertsHTML}</td><td class="p-4 text-center"><button onclick="event.stopPropagation(); window.openDocsModal('${o.id}')" class="bg-white border border-gray-300 text-gray-600 hover:bg-gray-50 hover:text-brand-dark px-3 py-1.5 rounded-lg text-xs font-bold transition shadow-sm flex items-center gap-2 mx-auto"><i class="fa-solid fa-folder-open text-brand-red"></i> Expediente</button></td><td class="p-4 text-center"><button onclick="window.askDeleteOrder('${o.id}', event)" class="text-gray-400 hover:text-red-600"><i class="fa-solid fa-trash"></i></button></td>`;
        t.appendChild(tr);
    });
}
...
// EDICIÓN EN LÍNEA PARA CONCEPTOS LOGÍSTICOS
window.updateConceptAmount = function(index, newVal) {
    currentConcepts[index].amount = parseFloat(newVal) || 0;
    currentConcepts[index].value = parseFloat(newVal) || 0;
    window.recalcTotal();
};
window.updateConceptName = function(index, newName) {
    currentConcepts[index].description = newName;
};
...
window.renderConceptsList = function() { 
    ...
    if (!currentConcepts || currentConcepts.length === 0) { tbody.innerHTML = '<tr><td colspan="3" class="p-3 text-center text-gray-400 italic text-[10px]">Sin conceptos adicionales.</td></tr>'; return; }
    ...
    currentConcepts.forEach((c, idx) => {
        ...
        const valCol = isLocked ? `$${val.toLocaleString()}` : `$<input type="number" value="${val}" class="w-20 bg-transparent border-b border-transparent hover:border-gray-200 focus:border-brand-red outline-none text-right font-bold transition" onchange="window.updateConceptAmount(${idx}, this.value)">`;
...
window.recalcTotal = function() { 
    ...
    let concepts = 0; (currentConcepts || []).forEach(c => { concepts += parseFloat(c.amount || c.value || 0); }); let sub = base + concepts;
...
window.addConceptRow = function() { 
    ...
    if (id) {
        ...
        currentConcepts.push({ description: c.nombre, amount: amount || c.precio_sugerido, value: amount || c.precio_sugerido, unit: 'fixed', type: 'aumento' });
    } else {
        currentConcepts.push({ description: 'Concepto libre (Modificar)', amount: amount, value: amount, unit: 'fixed', type: 'aumento' });
    }
...
window.initiateApprovalSnapshot = async function() {
    ...
    if (!formData.numero_orden) { const date = new Date().toISOString().slice(0,10).replace(/-/g,''); const rnd = Math.floor(1000 + Math.random() * 9000); formData.numero_orden = `ORD-${date}-${rnd}`; }
    const content = window.getOrderHTML({ ...currentPreviewOrder, ...formData }, 'quote'); const pdfContainer = document.getElementById('pdf-content'); const embedViewer = document.getElementById('doc-preview'); const btnAction = document.getElementById('btn-download-preview');
...
window.executeApprovalTransaction = async function(formData) {
    ...
    try {
        ...
        const path = `${currentPreviewOrder.id}/cotizacion_aprobada_${Date.now()}.pdf`; await window.globalSupabase.storage.from('documentos-cp').upload(path, pdfBlob);
...
window.getFormDataFromModal = function() {
    ...
    return {
        ...
        conceptos_adicionales: currentConcepts, desglose_precios: { subtotal_antes_impuestos: sub, impuestos_detalle: activeTaxIds }, personas: guests 
    };
};
...
window.processSaveOrder = async function() {
    ...
    try { const formData = window.getFormDataFromModal(); formData.status = document.getElementById('oed-status').value; const { error } = await window.finSupabase.from('cotizaciones').update(formData).eq('id', document.getElementById('oed-id').value); if (error) throw error; window.showToast("Cambios guardados", "success"); window.closeModal('order-edit-modal'); await window.loadOrders(); } catch(e) { window.showToast("Error: " + e.message, "error"); } finally { btn.disabled = false; btn.innerText = "Guardar"; }
};
...
window.downloadPDFFromPreview = function() { const element = document.getElementById('pdf-content'); const opt = { margin: 0, filename: `Documento.pdf`, image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2, useCORS: true }, jsPDF: { unit: 'in', format: 'letter' } }; html2pdf().set(opt).from(element).save(); };

window.getOrderHTML = function(o, type) { 
    const isOrder = type === 'order'; const logoImg = `<img src="${COMPANY_LOGO_URL}" class="h-16 object-contain" crossorigin="anonymous">`; const now = new Date(); const dateStr = now.toLocaleDateString('es-MX', { day: '2-digit', month: '2-digit', year: 'numeric' }); const genDateTime = now.toLocaleString('es-MX', { dateStyle: 'short', timeStyle: 'medium' }); let docTitle = isOrder ? "ORDEN DE COMPRA" : "COTIZACIÓN"; let folio = o.numero_orden || (isOrder ? 'PENDIENTE' : o.id.slice(0,8).toUpperCase()); const space = allSpaces.find(s=>s.id==o.espacio_id); const descHTML = isOrder ? '' : `<p class="text-[9px] text-gray-500 italic mt-0.5 truncate max-w-xs">${space?.descripcion || ''}</p>`; const footerHubHTML = `<div class="w-full text-center mt-10"><p class="text-[10px] text-gray-400 font-medium leading-tight">Generado el ${genDateTime}<br>a través de Marketing Hub</p></div>`; const renderHeader = (title) => `<div class="flex justify-between items-start border-b-4 border-brand-red pb-3 mb-2">${logoImg}<div class="text-right"><h1 class="text-2xl font-black text-gray-800 tracking-tighter uppercase">${title}</h1><p class="text-sm font-mono text-brand-red font-bold mt-1">${folio}</p><p class="text-[10px] text-gray-500 mt-1">${dateStr}</p></div></div>`; 
    ...
    cArray.forEach(c => { let val = parseFloat(c.amount !== undefined ? c.amount : (c.value || 0)); let amount = val; if(c.unit === 'percent') amount = rentalTotal * (val/100); if(c.type === 'descuento') runningSubtotal -= amount; else runningSubtotal += amount; const sign = (c.type === 'descuento') ? '-' : '+'; rowsHtml += `<tr><td class="py-2 px-3 text-xs text-gray-600">${c.description || c.nombre || 'Adicional'}</td><td class="py-2 px-3"></td><td class="py-2 px-3 text-right text-xs text-gray-600">${sign} ${new Intl.NumberFormat('es-MX', {style:'currency',currency:'MXN'}).format(amount)}</td></tr>`; }); 
