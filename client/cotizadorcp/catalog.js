...
async function loadClientProfilesForQuoteModal() {
    ...
    try {
        ...
        clientProfiles = data || []; clientProfilesById = {}; clientProfiles.forEach(c => clientProfilesById[c.id] = c);
        ...
        sel.onchange = () => {
            ...
            const c = clientProfilesById[id]; if (!c) return;
            if (hid) hid.value = id;
            const n = document.getElementById('cli-name'); const p = document.getElementById('cli-phone'); const e = document.getElementById('cli-email'); const r = document.getElementById('cli-rfc');
            if (n) n.value = c.nombre_completo || ''; if (p) p.value = (c.telefono || ''); if (e) e.value = (c.correo || ''); if (r) r.value = (c.rfc || '');
        };
        const clearAssoc = () => { if (sel.value) sel.value = ''; if (hid) hid.value = ''; };
        ['cli-name','cli-phone','cli-email','cli-rfc'].forEach(id => { const el = document.getElementById(id); if (el) el.addEventListener('input', clearAssoc); });
...
function calculateDayByDayTotal(space, startStr, endStr, guests) {
    ...
    const start = new Date(startStr + 'T00:00:00'); const end = new Date(endStr + 'T00:00:00'); const keys = ['domingo', 'lunes', 'martes', 'miercoles', 'jueves', 'viernes', 'sabado'];
    ...
    for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) { const key = keys[d.getDay()]; let price = parseFloat(prices[key] || 0); if (blockedDays.includes(key)) price = 0; total += price; }
    return { total };
}
...
window.openManagerModal = function(id){
    ...
    if(id) { 
        ...
        let rules = []; if (s.precios_por_dia) { if (Array.isArray(s.precios_por_dia)) rules = s.precios_por_dia; else rules = [{min: 1, max: 9999, precios: s.precios_por_dia}]; } else { rules = [{min: 1, max: 100, precios: {lunes:0, martes:0, miercoles:0, jueves:0, viernes:0, sabado:0, domingo:0}}]; }
        ...
        // CARGA DE RELOJES LOGÍSTICA B2B
        let b2b = {}; try { b2b = typeof s.config_b2b === 'string' ? JSON.parse(s.config_b2b) : (s.config_b2b || {}); } catch(e){}
        const h = b2b.horarios || {};
        document.getElementById('cfg-precio-montaje').value = b2b.precio_montaje || 0;
        document.getElementById('cfg-precio-hora').value = b2b.precio_hora_extra || 0;
        
        ['mat','ves','noc','tod'].forEach(k => {
            const fullKey = k==='mat'?'matutino':k==='ves'?'vespertino':k==='noc'?'nocturno':'todo_dia';
            document.getElementById(`cfg-${k}-inicio`).value = h[fullKey]?.start || '';
            document.getElementById(`cfg-${k}-fin`).value = h[fullKey]?.end || '';
            document.getElementById(`cfg-${k}-precio`).value = h[fullKey]?.price || 0;
        });
        ...
        document.getElementById('mgr-title').innerText = "Nuevo Espacio"; document.getElementById('mgr-key').value = ''; document.getElementById('mgr-key').disabled = false; document.getElementById('mgr-name').value = ''; document.getElementById('mgr-tags').value = ''; document.getElementById('mgr-desc').value = ''; window.addRangeRow();
        
        ['mat','ves','noc','tod'].forEach(k => { document.getElementById(`cfg-${k}-inicio`).value = ''; document.getElementById(`cfg-${k}-fin`).value = ''; document.getElementById(`cfg-${k}-precio`).value = 0; });
        document.getElementById('cfg-precio-montaje').value = 0; document.getElementById('cfg-precio-hora').value = 0;

        document.getElementById('mgr-active').checked = true; document.getElementById('btn-delete-mgr').classList.add('hidden'); document.getElementById('mgr-preview').src = ''; document.getElementById('mgr-preview').classList.add('hidden');
    } 
...
window.saveSpace = async function(){ 
    ...
    const btn = document.getElementById('btn-save-mgr'); btn.disabled = true; btn.innerText = "Guardando...";
    ...
    try {
        ...
        // GUARDADO B2B EXACTO
        const b2bConfig = {
            precio_montaje: parseFloat(document.getElementById('cfg-precio-montaje').value) || 0,
            precio_hora_extra: parseFloat(document.getElementById('cfg-precio-hora').value) || 0,
            horarios: {
                matutino: { start: document.getElementById('cfg-mat-inicio').value, end: document.getElementById('cfg-mat-fin').value, price: parseFloat(document.getElementById('cfg-mat-precio').value) || 0 },
                vespertino: { start: document.getElementById('cfg-ves-inicio').value, end: document.getElementById('cfg-ves-fin').value, price: parseFloat(document.getElementById('cfg-ves-precio').value) || 0 },
                nocturno: { start: document.getElementById('cfg-noc-inicio').value, end: document.getElementById('cfg-noc-fin').value, price: parseFloat(document.getElementById('cfg-noc-precio').value) || 0 },
                todo_dia: { start: document.getElementById('cfg-tod-inicio').value, end: document.getElementById('cfg-tod-fin').value, price: parseFloat(document.getElementById('cfg-tod-precio').value) || 0 }
            }
        };
        ...
        if(fileInput.files && fileInput.files.length > 0) {
            const file = fileInput.files[0]; const fileExt = file.name.split('.').pop(); const filePath = `espacios/${Date.now()}.${fileExt}`;
            const { error } = await window.globalSupabase.storage.from('Espacios').upload(filePath, file); if(error) throw error;
            imgUrl = window.globalSupabase.storage.from('Espacios').getPublicUrl(filePath).data.publicUrl;
        }

        const payload = { clave: document.getElementById('mgr-key').value.toUpperCase().trim(), nombre: document.getElementById('mgr-name').value, tipo: document.getElementById('mgr-type').value, descripcion: document.getElementById('mgr-desc').value, precio_base: maxPriceFound, precios_por_dia: ranges, dias_bloqueados: blockedDays, config_b2b: b2bConfig, etiquetas: tagsArray, ajuste_tipo: document.getElementById('mgr-adj-type').value, ajuste_porcentaje: parseFloat(document.getElementById('mgr-adj-pct').value) || 0, activa: document.getElementById('mgr-active').checked, impuestos_ids: selectedTaxes, imagen_url: imgUrl }; 
        if(id) { await window.finSupabase.from('espacios').update(payload).eq('id', id); } else { await window.finSupabase.from('espacios').insert(payload); } 
        window.showToast("Guardado", "success"); window.closeModal('manager-modal'); loadCatalog(); fileInput.value = '';

    } catch(e) { window.showToast("Error: " + e.message, "error"); } finally { btn.disabled = false; btn.innerText = "Guardar"; }
}

// -------------------------------------------------------------
// COTIZADOR B2B DE ADMINISTRACIÓN
// -------------------------------------------------------------
...
window.openQuoteModal = function(id) {
    ...
    // CARGAR HORARIOS B2B AL SELECT
    let b2b = {}; try { b2b = typeof currentSpace.config_b2b === 'string' ? JSON.parse(currentSpace.config_b2b) : (currentSpace.config_b2b || {}); } catch(e){}
    const h = b2b.horarios || {}; const selHorario = document.getElementById('q-horario'); selHorario.innerHTML = '';
    const mapNames = { matutino: 'Matutino', vespertino: 'Vespertino', nocturno: 'Nocturno', todo_dia: 'Todo el día' };
    ['matutino', 'vespertino', 'nocturno', 'todo_dia'].forEach(k => {
        if(h[k] && h[k].start && h[k].end) selHorario.innerHTML += `<option value="${k}" data-price="${h[k].price}">${mapNames[k]} (${h[k].start} a ${h[k].end})</option>`;
    });
    if(selHorario.innerHTML === '') selHorario.innerHTML = '<option value="nocturno" data-price="0">Nocturno (Por definir)</option>';

    // CARGAR CONCEPTOS CATALOGO
    ...
    adminSelectedConcepts = []; window.updateAdminConceptsSummary();
...
window.addAdminConcept = function() {
    const sel = document.getElementById('admin-concept-select'); const id = sel.value; if(!id) return;
    const concept = catalogConcepts.find(c => c.id == id);
    if(concept) {
        adminSelectedConcepts.push({ description: concept.nombre, amount: concept.precio_sugerido, value: concept.precio_sugerido, unit: 'fixed', type: 'aumento' });
        window.updateAdminConceptsSummary(); window.updateQuoteCalculation();
    }
    sel.value = '';
}
...
window.updateAdminConceptsSummary = function() {
    const container = document.getElementById('admin-concepts-summary'); container.innerHTML = '';
    adminSelectedConcepts.forEach((c, idx) => {
        container.innerHTML += `<div class="flex justify-between items-center bg-gray-50 border border-gray-100 p-2 rounded text-xs"><span class="font-bold text-gray-700">${c.description}</span><div class="flex items-center gap-3"><span class="font-black text-brand-dark">$${parseFloat(c.amount).toLocaleString()}</span><button onclick="window.removeAdminConcept(${idx})" class="text-gray-400 hover:text-red-500"><i class="fas fa-times"></i></button></div></div>`;
    });
}
...
window.generatePDF = async function() {
    ...
    let conceptosB2B = [];
    if(selOpt) conceptosB2B.push({ description: `Horario: ${selOpt.text}`, amount: costoHorario, value: costoHorario, unit: 'fixed', type: 'aumento' });
    if(diasM > 0) conceptosB2B.push({ description: `Montaje (${diasM} días)`, amount: (diasM * pMontaje), value: (diasM * pMontaje), unit: 'fixed', type: 'aumento' });
    if(hrsE > 0) conceptosB2B.push({ description: `Horas Extras (${hrsE} hrs)`, amount: (hrsE * pHora), value: (hrsE * pHora), unit: 'fixed', type: 'aumento' });
    ...
    const payload = { 
        ...
        desglose_precios: { subtotal_antes_impuestos: currentPricing.subtotal, impuestos_detalle: parseIds(currentSpace.impuestos_ids), tax_total: currentPricing.taxes }, 
        conceptos_adicionales: conceptosB2B, status: 'pendiente', 
