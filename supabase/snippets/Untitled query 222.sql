...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // --- SISTEMA DE PERMISOS DE NAVEGACIÓN (NUEVO) ---
    if (userRole !== 'admin') {
        const navRules = {
            'orders.html': myPermissions.orders_view,
            'reports.html': myPermissions.reports_view
        };
...
function renderSpaces(list) { 
    ...
    list.forEach(s => { 
        ...
        let displayImg = 'https://via.placeholder.com/400x300?text=Sin+Imagen';
